...
class TestPOSIXPaths(unittest.TestCase):
    ...
    def test_manifest_uses_posix_paths(self):
        ...
        for file_entry in manifest.get("files", []):
            ...
            self.assertNotIn("\\", path, 
                f"Path contains backslash (non-POSIX): {path}")
            
            # Verify forward slashes are used for subdirectories
            if "/" in path:
                self.assertTrue(path.count("/") > 0,
                    f"Path should use forward slashes: {path}")
            
            # Verify path doesn't start with absolute marker
            self.assertFalse(path.startswith("/"),
                f"Path should be relative, not absolute: {path}")
            
            # Verify no drive letters (Windows C:\ etc)
            self.assertFalse(":" in path,
                f"Path contains drive letter or colon: {path}")
    ...
    def test_manifest_paths_are_relative(self):
        ...
        for file_entry in manifest.get("files", []):
            ...
            self.assertTrue(
                path.startswith("KASPERMLX/"),
                f"Path should be relative to repo root: {path}"
            )
            
            # Should not contain parent directory references
            self.assertNotIn("..", path,
                f"Path should not contain parent directory references: {path}")
        
        print(f"✅ All paths are properly relative to repo root")
