...
class OPUSBatchConverter:
    ...
    def create_fallback_grok_structure(self, content: str, number: int, persona: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for line in content.split('\n'):
                if line.startswith('###'):
                    ...
                    current_section = line.strip('#').strip().lower().replace(' ', '_').replace('-', '_')
                    current_content = []
    ...
    def extract_keywords_from_sections(self, sections: Dict[str, str]) -> List[str]:
        ...
        keywords = set()
        spiritual_terms = ['monad', 'chakra', 'divine', 'sacred', 'mystical', 'spiritual', 'consciousness', 
                          'enlightenment', 'meditation', 'energy', 'vibration', 'cosmic', 'universal']
    ...
    def batch_convert_all(self):
        ...
        if chatgpt_path.exists():
            ...
            for file_path in sorted(chatgpt_path.glob("*.md")):
                ...
                if result:
                    output_file = self.output_path / f"chatgpt_original_{result['number']:02d}_converted.json"
        ...
        if claude_path.exists():
            ...
            for file_path in sorted(claude_path.glob("*.md")):
                ...
                if result:
                    output_file = self.output_path / f"claude_{result['number']:02d}_academic_converted.json"
                    self.save_and_optionally_replace(result, output_file, file_path)
        ...
        for persona in personas:
            ...
            if persona_path.exists():
                ...
                for file_path in sorted(persona_path.glob("*.md")):
                    ...
                    if result:
                        output_file = self.output_path / f"grok_{persona.lower()}_{result['number']:02d}_converted.json"
                        self.save_and_optionally_replace(result, output_file, file_path)
        ...
        print(f"📈 Total: {sum(conversion_stats.values()) - conversion_stats['errors']} JSON files created!")
        
        return conversion_stats
    
    def save_and_optionally_replace(self, data: Dict[str, Any], output_file: Path, original_file: Path):
...
def main():
    ...
    stats = converter.batch_convert_all()
