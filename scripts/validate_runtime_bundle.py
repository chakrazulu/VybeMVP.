...
class RuntimeBundleValidator:
    ...
    def _validate_manifest(self) -> bool:
        ...
        try:
            ...
            for logical_field, possible_names in required_fields.items():
                names_to_check = possible_names if isinstance(possible_names, list) else [possible_names]
                found = any(name in self.manifest for name in names_to_check)
                if not found:
                    self.errors.append(f"Missing required manifest field: {logical_field} (looked for: {names_to_check})")
    ...
    def _validate_fallback_chain(self) -> bool:
        ...
        fallback_strategy = (
            self.manifest.get('fallbackStrategy') or 
            self.manifest.get('fallback_strategy', 'unknown')
        )
        ...
        if missing_master_numbers:
            self.warnings.append(f"Master numbers missing content: {sorted(missing_master_numbers)}")
