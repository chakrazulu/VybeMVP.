...
class ContentCoverageAnalyzer:
    ...
    def analyze(self) -> ContentCoverageReport:
        ...
        # Only print console output if not generating JSON
        if hasattr(self, "_output_format") and self._output_format != "json":
    ...
    def _calculate_priority_score(
        self, number: int, rich_exists: bool, missing_contexts: List[str]
    ) -> int:
    ...
    def _calculate_overall_stats(self, number_details: List[NumberCoverage]) -> CoverageStats:
        ...
        for detail in number_details:
            ...
            if detail.is_master_number and (
                not detail.rich_content_exists or detail.missing_contexts
            ):
    ...
    def _generate_recommendations(
        self, number_details: List[NumberCoverage], stats: CoverageStats
    ) -> List[str]:
        ...
        if stats.coverage_percentage < 50:
            recommendations.append(
                "🚨 CRITICAL: Content coverage below 50% - immediate action required"
            )
        ...
        if "lifePath" in stats.missing_contexts:
            recommendations.append(
                "📍 FOCUS: LifePath content gaps detected - primary spiritual journey affected"
            )

        if "soulUrge" in stats.missing_contexts:
            recommendations.append(
                "💫 FOCUS: SoulUrge content gaps detected - inner motivation insights affected"
            )
    ...
    def generate_console_report(self) -> str:
        ...
        for detail in self.report.number_details[:10]:
            ...
            lines.append(
                f"  {master_icon}Number {detail.number:2d}: {status} (Priority: {detail.priority_score})"
            )
    ...
    def generate_markdown_report(self) -> str:
        ...
        for detail in self.report.number_details[:15]:
            ...
            missing_contexts = (
                ", ".join(detail.missing_contexts) if detail.missing_contexts else "None"
            )

            lines.append(
                f"| {detail.number} | {number_type} | {rich_status} | {missing_contexts} | {detail.priority_score} |"
            )
...
def main():
    ...
    parser.add_argument(
        "--bundle-path", default="KASPERMLXRuntimeBundle", help="Path to RuntimeBundle directory"
    )
    ...
    try:
        ...
        analyzer._output_format = args.format  # Set format for output control
        analyzer.analyze()
