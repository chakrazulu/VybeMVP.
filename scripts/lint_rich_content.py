...
class RichContentLinter:
    ...
    def __init__(self, project_root: Path = None):
        ...
        self.single_numbers_dir = self.project_root / "KASPERMLX/MLXTraining/ContentRefinery/SingleNumbers"
        self.master_numbers_dir = self.project_root / "KASPERMLX/MLXTraining/ContentRefinery/MasterNumbers"
    ...
    def validate_content_file(self, file_path: Path, fix_mode: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if artifacts:
                result["artifacts_found"] = [f"Pattern: {pattern}, Found: '{text[:50]}...'" 
                                           for pattern, text in artifacts]
            ...
            try:
                ...
                result["errors"].append(f"Schema violation: {e.message} at {'.'.join(str(x) for x in e.absolute_path)}")
    ...
    def _validate_intensity_scores(self, data: Dict, result: Dict) -> None:
        ...
        try:
            ...
            if not (0.0 <= min_range <= 1.0):
                result["errors"].append(f"intensity_scoring.min_range ({min_range}) not in [0.0, 1.0]")
            
            if not (0.0 <= max_range <= 1.0):
                result["errors"].append(f"intensity_scoring.max_range ({max_range}) not in [0.0, 1.0]")
            ...
            for i, insight in enumerate(insights):
                ...
                if not (0.0 <= intensity <= 1.0):
                    result["errors"].append(f"behavioral_insights[{i}].intensity ({intensity}) not in [0.0, 1.0]")
                elif not (min_range <= intensity <= max_range):
                    result["warnings"].append(f"behavioral_insights[{i}].intensity ({intensity}) outside expected range [{min_range}, {max_range}]")
    ...
    def _validate_insight_quality(self, data: Dict, result: Dict) -> None:
        ...
        try:
            ...
            if len(insights) != 20:
                result["errors"].append(f"Expected exactly 20 behavioral_insights, found {len(insights)}")
            ...
            for i, insight in enumerate(insights):
                ...
                if len(insight_text) < 50:
                    result["errors"].append(f"behavioral_insights[{i}].insight too short ({len(insight_text)} chars, minimum 50)")
                elif len(insight_text) > 1000:
                    result["warnings"].append(f"behavioral_insights[{i}].insight very long ({len(insight_text)} chars)")
                
                # Check for artifacts in insight text  
                artifacts = self.detect_claude_artifacts(insight_text)
                if artifacts:
                    result["errors"].append(f"behavioral_insights[{i}].insight contains Claude artifacts: {[a[1] for a in artifacts]}")
    ...
    def _validate_completeness(self, data: Dict, result: Dict) -> None:
        ...
        try:
            ...
            if len(affirmations) < 5:
                result["errors"].append(f"meta.affirmations needs at least 5 items, found {len(affirmations)}")
            
            # Keywords count  
            keywords = meta.get("keywords", [])
            if len(keywords) < 5:
                result["errors"].append(f"meta.keywords needs at least 5 items, found {len(keywords)}")
            
            # Rituals count
            rituals = meta.get("rituals", [])
            if len(rituals) < 3:
                result["errors"].append(f"meta.rituals needs at least 3 items, found {len(rituals)}")
    ...
    def generate_report(self) -> str:
        ...
        for file_result in self.results["error_details"]:
            ...
            if file_result["artifacts_found"]:
                report.append(f"  🧹 Claude artifacts detected:")
...
Examples:
  ...
  python3 scripts/lint_rich_content.py --path single_1    # Validate specific file
    ...
    parser.add_argument(
        "--path", 
        help="Specific file to validate (e.g., single_1, master_11)"
    )
    parser.add_argument(
        "--fix", 
        action="store_true",
    ...
    parser.add_argument(
        "--strict",
        action="store_true", 
        help="Enable extra strict validation rules"
    )
