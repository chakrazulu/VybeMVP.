...
class KASPERReleaseCardsGenerator:
    ...
    def __init__(self, dataset_name=None, dataset_version=None, soft_mode=False, policy_path=None):
        ...
        self.schema_file = (
            self.project_root / "NumerologyData/PERFECT_EXPRESSION_NUMBER_SCHEMA.json"
        )
        self.output_dir = self.project_root / "docs"
        self.artifacts_dir = self.project_root / "artifacts"
        self.policy_file = (
            Path(policy_path) if policy_path else (self.project_root / "configs/release_policy.yml")
        )
    ...
    def _load_release_policy(self) -> Dict:
        ...
        policy_explicitly_requested = str(self.policy_file) != str(
            self.project_root / "configs/release_policy.yml"
        )
        ...
        try:
            if self.policy_file.exists() and HAS_YAML:
                ...
                if policy_explicitly_requested and not self.soft_mode:
                    ...
                    raise ImportError(
                        "PyYAML is required for custom policy files in hard mode. Install with: pip install pyyaml"
                    )
                else:
    ...
    def _handle_validation_issue(
        self, gate_name: str, message: str, is_critical: bool = False
    ) -> bool:
        ...
        should_warn_only = (
            self.soft_mode
            and self.policy.get("local_development", {}).get("soft_mode_available", False)
            and gate_name in warning_only_gates
    ...
    def validate_against_schema(self, categories: Dict[str, List[Path]]) -> Dict[str, Any]:
        ...
        # TODO: Implement schema validation once schema format is finalized
        if self.schema_file.exists():
            try:
                with open(self.schema_file, "r", encoding="utf-8") as f:
                    _schema = json.load(f)  # Load but don't use yet
            ...
            for file_path in files:
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        _data = json.load(f)  # Load but don't analyze structure yet
                    ...
                    validation_results["errors"].append({"file": file_path.name, "error": str(e)})

            validation_results["schema_compliance"][category] = category_stats

        print(
            f"✅ Validation complete: {validation_results['valid_files']}/{validation_results['total_files']} files valid"
        )
    ...
    def analyze_content_statistics(self, categories: Dict[str, List[Path]]) -> Dict[str, Any]:
        ...
        for category, files in categories.items():
            ...
            if category_stats["file_count"] > 0:
                category_stats["avg_file_size"] = (
                    category_stats["total_bytes"] / category_stats["file_count"]
                )
    ...
    def generate_manifest(self, categories: Dict[str, List[Path]]) -> Dict[str, Any]:
        ...
        print(
            f"🔐 Manifest generated: {manifest['total_files']} files, dataset digest: {manifest['dataset_digest'][:12]}..."
        )
        return manifest
    ...
    def validate_schema_compliance(self, categories: Dict[str, List[Path]]) -> Dict[str, Any]:
        ...
        print(
            f"✅ Hard validation passed: {validation_results['valid_files']}/{validation_results['total_files']} files ({success_rate:.1f}%)"
        )

        return validation_results

    def generate_data_card(
        self, categories: Dict[str, List[Path]], stats: Dict[str, Any], validation: Dict[str, Any]
    ) -> str:
...
"""
    ...
    def generate_release_documentation(self):
        ...
        try:
            ...
            bundle_path = self.create_release_bundle(
                data_card, model_card, categories, stats, validation, manifest
            )

            print("\n🎉 KASPER MLX Release Documentation Complete!")
            print(
                f"📊 Dataset: {stats['file_count']} files, {stats['total_size_bytes']/(1024*1024):.2f} MB"
            )
            print(
                f"✅ Validation: {validation['valid_files']}/{validation['total_files']} files pass ({validation['valid_files']/validation['total_files']*100:.1f}%)"
            )
            print(f"📦 Bundle: {bundle_path}")
            print("🔮 Ready for MLX training and production deployment!")
...
Examples:
  ...
  python make_release_cards.py --version v2.1    # Custom version string
    ...
    parser.add_argument(
        "--name", default=None, help="Override dataset name (default: kasper-lp-trinity)"
    )
    ...
    parser.add_argument("--policy", default=None, help="Path to custom release policy YAML file")
    ...
    try:
        ...
        if generator.validation_errors and not args.soft:
            ...
            if args.soft and generator.validation_errors:
                print(
                    f"🔧 Note: {len(generator.validation_errors)} errors were downgraded to warnings in soft mode"
                )
            sys.exit(0)
